    ...
    public class CampsController : BaseController
    {
        ...
        public async Task<IActionResult> Post([FromBody]CampModel model)
        {
            try
            {
                ...
                var newCamp = _mapper.Map<Camp>(model);
                ...
                {
                    var newUri = Url.Link("CampGet", new { moniker = newCamp.Moniker });
                    return Created(newUri, _mapper.Map<CampModel>(newCamp));
                }
                else
                {
        ...
        public async Task<IActionResult> Put(string moniker, [FromBody] CampModel model)
        {
            try
            {
                ...
                if (oldCamp == null) return NotFound($"Could not found a Camp with a moniker of {moniker}");
            ...
            {
                _logger.LogError($"Threw exception while updating Camp (moniker = {moniker}), exception : {ex}");
            }
        ...
        public async Task<IActionResult> Delete(string moniker)
        {
            ...
            {
                _logger.LogError($"Threw exception while deleting Camp (id = {moniker}), exception : {ex}");
            }
